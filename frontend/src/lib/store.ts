...
import { cookieStorage, clearAllAppCookies } from "./cookie-storage";
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      // Actions
      setUser: (user) => {
        // Clear existing cookies before setting new user data
        if (user) {
          clearAllAppCookies();
        }
      ...
      updateAdminDocument: (id, updates) =>
        set((state) => ({
      ...
      logout: () => {
        // Clear all app-related cookies first
        clearAllAppCookies();
